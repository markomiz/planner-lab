...
class dubinCurve
{
  ...
  geometry_msgs::msg::PoseStamped pose2dToPose(pose2d pose) // DONE?
  {
  ...
  pose2d nextPointOnArc(pose2d x0, float ds, int k) // DONE
  {
    pose2d next = x0;
  ...
  nav_msgs::msg::Path generatePathFromDubins(pose2d start, std::vector<dubins_params> sub_paths, float delta) // DONE
  {
  ...
  std::vector<dubins_params> calculateMultiPoint(pose2d start, pose2d end, std::vector<point2d> mid_points, int n_angles) // DONE
  {
  ...
  dubins_params DUBINATOR( float th0, float th1, float lambda, ksigns ks ) // DONE
  {
    dubins_params path;
    path.k = ks;
