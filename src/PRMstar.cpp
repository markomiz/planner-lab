...
void PRMstar::genRoadmap(int n)
{
    ...
    for (auto i = 0; i < n; i ++)
    {
        ...
        graph->points_quad.add_node(new_node);
    }
...
deque<arcs> PRMstar::getPath(pose2d start, pose2d end)
{
    ...
    {
        dubins_params sol = dCurve->calculateSinglePath( nearest_e[y]->pt, end);
        arcs A(nearest_e[y]->pt, sol);
        // if doesn't collide add connections to graph
        if (map->colliding(A)) continue;
        graph->add(nearest_e[y], end_node,  A);
    };
